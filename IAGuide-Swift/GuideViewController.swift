...
//booleans
    ...
    var scheduleController: ClassBlockController!
//school events
    ...
    override func viewDidLoad() {
        ...
        scheduleController = ClassBlockController()
        addChildViewController(scheduleController)
        view.addSubview(scheduleController.view)
        ...
        isADay = DayManager.sharedClassManager().todayIsAnADay(NSDate())
    ...
    func switchDayImage() {
        if finishedAnimation {
            ...
            Global.dateFormatter().dateFormat = "e"//day of week format (1-7)
            var dayOfWeek = Global.dateFormatter().stringFromDate(now).toInt()
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        Global.dateFormatter().dateStyle = NSDateFormatterStyle.MediumStyle
        
        if schoolEvent.eventLocation != nil {
            cell.detailTextLabel?.text = "\(Global.dateFormatter().stringFromDate(eventStart)), \(eventLocation)"
        } else {
            cell.detailTextLabel?.text = "\(Global.dateFormatter().stringFromDate(eventStart))"
        }
    ...
    func updateEventsArray() {  //when the events calendar is updated, filter the data
        ...
        for _ in 1...7 {
            ...
            Global.dateFormatter().dateFormat = "d"//day of month format
            let dayOfMonth = Global.dateFormatter().stringFromDate(date).toInt()
            Global.dateFormatter().dateFormat = "M"//month format
            let monthNumber = Global.dateFormatter().stringFromDate(date).toInt()
            for event in eventsCalendar.events {
                if let eventStart = event.eventStartDate {
                    Global.dateFormatter().dateFormat = "M"//reset to month format
                    if Global.dateFormatter().stringFromDate(eventStart).toInt() == monthNumber {
                        Global.dateFormatter().dateFormat = "d"//day format
                        let f = Global.dateFormatter().stringFromDate(eventStart).toInt()
                        if f == dayOfMonth {
                            let x = 9
